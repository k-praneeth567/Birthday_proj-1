...
const OPTIONS = ['A river', 'A clock', 'A shadow', 'Time']
const CORRECT = 'A clock'
  ...
  return (
    <div className="text-center">
      <motion.h2 className="font-script text-3xl text-[#ff4b9f] mb-4"
        initial={{opacity:0,y:8}} animate={{opacity:1,y:0}}>Level 2</motion.h2>

      <motion.div className="text-sm text-slate-200 mb-5 leading-relaxed"
        initial={{opacity:0,y:8}} animate={{opacity:1,y:0}}>
        I’m always running but never move.<br/>
        I have a face but no eyes, and hands but no arms.<br/>
        What am I?
      </motion.div>
      ...
      {answered && (
        <motion.p initial={{opacity:0}} animate={{opacity:1}} transition={{duration:0.45}} className="mt-6 text-sm text-[#ffdfa9]">
          Time stops when I’m with you…<br/>
          <span className="italic">and tonight, I don’t want it to start again.</span>
        </motion.p>
      )}
    </div>
  )
